...
\paragraph{EarnHFT (\cref{ch:earnhft}).} For second-level high-frequency trading, EarnHFT combines a sample-path optimal Q-teacher with trend-conditioned policies and an offline minute-level router. Across four dense second-level replays, EarnHFT attains the highest total return on all four datasets, and the component ablation supports the usefulness of the optimal Q-teacher and optimal actor.

\paragraph{FineFT (\cref{ch:fineft}).} For leveraged futures, FineFT uses an ensemble TD matrix to assign updates selectively, fits a VAE to each inferred dynamic, and routes by reconstruction-based familiarity scores with a conservative fallback. Across the four crypto markets, FineFT has the highest average total return among learned methods and the lowest average maximum drawdown. The selective-update and routing ablations further support the two principal components: selective training improves the efficiency--performance trade-off, while the VAE router and risk threshold combine complementary specialists and reduce downside exposure.
...
\paragraph{FineFT.} The VAE reconstruction score measures distributional familiarity rather than a calibrated probability of policy failure. The supplementary Corn/ES results also use a lower frequency than the main cryptocurrency experiments. The results establish the value of the framework under the reported protocols, while broader frequency and asset-class transfer remain open questions.
