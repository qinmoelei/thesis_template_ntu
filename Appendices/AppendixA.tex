...
% LEGACY, UNCOMPILED SOURCE. mythesis.tex intentionally does not include this file.
% Chapters 4--6 contain the maintained statements; do not treat the duplicated
% derivations below as submission text without a separate author decision and
% a full synchronization pass.

\chapter{Legacy Proofs and Derivations (Not Included)} % Legacy appendix title
\label{AppendixA}

This file is a retired draft and is intentionally not included by \texttt{mythesis.tex}. The maintained statements and their limitations are in Chapters~\ref{ch:earnhft}--\ref{ch:alphaqd}; in particular, the released FineFT implementation does not support the liquidation-mask and convergence claims duplicated below. The material must not be restored to the submitted thesis without a separate author decision and a complete synchronization pass.
...
The bound rests on positive standalone edges, encouraged by the positive-ICIR admission rule, and on a uniform redundancy cap $\rho_{\max}$. The bounded per-cell quota and PCA descriptor encourage diversity but do not by themselves enforce that correlation premise. Theorem~\ref{app:thm_boost} shows that boosting-style fitting preserves neither premise, so no analogous bound is available for it.
